...
/* Exercise 4.5
   ...
   %, > (sin), < (cos), and ^ (power) operations. It can works 
   with negavite values and has specific operations: upper() ->
   pirnt upper number, dublicate() -> dublicate upper number, and 
...
/**** new operations: ****/
            case    '^':   /* to power */
                ...
                f = pow(pop(), op2);
                if (errno == EDOM) {
                    printf("IN ^ FUNCTION -> error of function definition area!\n");
                    return 0;
                }
                if (errno == ERANGE) {
